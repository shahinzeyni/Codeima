...
export default function Classes() {
  ...
  const findStudents = (studentId) => {
    return allUsers.filter((user) => studentId.includes(user.id))
  }
  ...
  const editUser = (event) => {
    ...
    if (userIsTeacher != -1) {
      formBody["teacher"] = userIsTeacher
    }
    if (courseCover == "[object File]") {
      formBody["image"] =  courseCover
    }
    if (addedStudent.length) {
      formBody["students"] = addedStudent
    }
      ...
      <div className={s.ticketsContents}>
        {allDataClasses.length == 0 ? (
          <>
              ...
              {allDataClasses.map((classData, index) => (
                <tbody className="w-full">
                  <tr key={classData.id}>
                    ...
                    <td>
                      <div className={s.tdSituationTicket}>
                          <select className={s.tdSituationTicket}>
                          <option >اسامی فراگیران</option>
            ...
            <div className="flex flex-col-reverse mt-16 justify-center items-center mx-auto w-[100%]">
              <span className="text-title text-[18px] mt-1 ml-2">
                هیچ ردیفی برای نمایش وجود ندارد.
              </span>
              <span>
                <svg
                  className="text-title text-[45px]"
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                  height="1em"
                  viewBox="0 0 16 16"
